...
// Contains returns `true` if key is part of the set with a false positive probability of <0.02%.
func (filter *Fuse16) Contains(key uint64) bool {
...
func (filter *Fuse16) makeKeyHashes(k uint64) hashes {
...
func (filter *Fuse16) geth012(hash uint64) h012 {
	answer := h012{}
	r0 := uint32(hash)
	r1 := uint32(rotl64(hash, 21))
	r2 := uint32(rotl64(hash, 42))
...
func PopulateFuse16(keys []uint64) (*Fuse16, error) {
	...
	filter.Fingerprints = make([]uint16, capacity, capacity)
